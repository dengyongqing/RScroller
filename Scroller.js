...
export default class Scroller extends Component {
    ...
    constructor(props) {
        ...
        window.setBoardHeight = this.setBoardHeight;
        window.setKeyboardHeight = this.setKeyboardHeight;
    }
    ...
    componentDidMount() {
        ...
        // this.iscroll = new IScroll(this.scroller);
    ...
    componentDidUpdate(prevProp, prevState) {
        ...
        if (!this.loadMoreed && !this.pullRefreshed) {
          const wrapperClientHeight = this.wrapper.clientHeight;
          const wrapScrollHeightAfter = this.wrapper.scrollHeight;
          if(!(this.scrollBottom.offsetTop < wrapScrollHeightAfter)) {
              this.scrollTo(0, 0, 0);
              this.scrollTo(0, -wrapScrollHeightAfter + wrapperClientHeight, 0);
          }
        }
    }
    ...
    touchStart(e) {
        ...
        if (this.useTransition && this.isInTransition) {
            ...
            // 停止滚动
            this.execEvent('onScrollEnd', this);
        ...
        this.execEvent('onBeforeScrollStart', this);
    }
    ...
    touchEnd(e) {
        ...
        if (this.state.usePullRefresh && this.y >= this.pullRefreshOffsetHeight) {
            if (this.pullRefreshStatus === REFRESHSTATUS.LOAD) {
                ...
                this.scrollTo(this.x, this.pullRefreshOffsetHeight, 0);
     ...
     * */
    loadData(type) {
        this.wrapScrollHeightBefore = this.wrapper.scrollHeight;
    ...
    loadDataHandle(type, code) {
        if (type === 'refresh') {
            ...
            setTimeout(() => {
                ...
                this.scrollTo(0, 0, 0);
                this.refresh();
                if (this.pullRefreshed) {
                  this.pullRefreshed = false;
                  const wrapScrollHeightAfter = this.wrapper.scrollHeight;
                  if (this.wrapScrollHeightBefore) {
                    this.scrollTo(0, -(wrapScrollHeightAfter - (this.wrapScrollHeightBefore || 0)));
                  }
                  // window.wrapScrollHeightBefore = this.wrapper.scrollHeight;
                }
            }, 0);
        } else {
        ...
        function step() {
            ...
            if (now >= destTime) {
                ...
                if (!self.resetPosition(self.props.bounceTime)) {
                    self.execEvent('onScrollEnd', this);
                }
                return;
            }
        ...
        return (
            ...
            >
                ...
                <div
                    ...
                    ref={ref => { this.scroller = ref; window.scroller = this.scroller;}}
                >
                    {
                        usePullRefresh ?
                            (<div
                                ref={ref => { this.pullRefreshEl = ref }}
                                className="silk-listcontrol-loadwrapper silk-listcontrol-loadwrapper-up testTest"
                            >
                                {this.state.refreshing ? <LoadingComponent size='1.5rem'/> : null}
                            </div>) : null
                    }
                    {React.Children.map(
                        children,
                ...
                <div ref={ref => (this.scrollBottom = ref)}></div>
            </div>
        );
    }
}
